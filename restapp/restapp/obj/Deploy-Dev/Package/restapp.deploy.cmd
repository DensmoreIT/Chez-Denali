...
set _DeploySetParametersFile=%RootPath%restapp.SetParameters.xml
)
...
@rem ---------------------------------------------------------------------------------
if not exist "%RootPath%restapp.zip" (
echo "%RootPath%restapp.zip" does not exist. 
...
echo. "%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%restapp.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension %_MsDeployAdditionalFlags%
"%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%restapp.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension %_MsDeployAdditionalFlags%
) else (
echo. "%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%restapp.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:"%RootPath%restapp.SetParameters.xml" %_MsDeployAdditionalFlags%
"%MSDeployPath%\msdeploy.exe" -source:package='%RootPath%restapp.zip' -dest:%_Destination% -verb:sync -disableLink:AppPoolExtension -disableLink:ContentExtension -disableLink:CertificateExtension -setParamFile:"%RootPath%restapp.SetParameters.xml" %_MsDeployAdditionalFlags%
)
goto :eof
...
:usage
echo =========================================================
if not exist "%RootPath%restapp.deploy-readme.txt" (
...
start notepad "%RootPath%restapp.deploy-readme.txt"
)
echo =========================================================
goto :eof
...
:CheckParameterFile
echo =========================================================
if  exist "%RootPath%restapp.SetParameters.xml" (
echo SetParameters from:
echo "%RootPath%restapp.SetParameters.xml"
echo You can change IIS Application Name, Physical path, connectionString
echo or other deploy parameters in the above file.
) else (
echo SetParamterFiles does not exist in package location.
